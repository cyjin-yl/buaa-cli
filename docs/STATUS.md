...
Issue #11 defines the safe gateway contract. The implementation uses only TLS-validated `https://gw.buaa.edu.cn` challenge, portal and usage paths. Usage is cache-only by default and redacts username/real-name/MAC response fields. Login requires exact typed stdin intent plus a prior typed `resume-auth`; the challenge and final credential submission are separate governor leases with the final `Authentication` request consuming the only armed attempt. Logout requires exact typed intent and `--online`. No request retries automatically.

Original Rust implements the XXTEA-style info field, custom base64, HMAC-MD5 password and SHA-1 checksum. An independent synthetic protocol vector pins expected output. Real loopback fixtures exercise usage normalization/cache reuse, challenge/login/logout with unchanged five-second spacing, no plaintext password/Cookie in requests, successful token consumption and credential-rejection latch behavior.

`cargo fmt --all --check`, strict all-target clippy and the final offline suite passed with **56 tests** and one ignored subprocess worker. Actual CLI fail-closed proof used only synthetic credentials: offline usage miss (7), valid login input without an armed attempt (5), invalid resume intent (2), and logout without explicit online mode (5). It performed zero campus requests and did not arm production authentication.
...
Independent contract review also corrected stale availability claims, preserved the unimplemented logout plan/commit/idempotency remainder, aligned username/password character and control bounds with discovery schemas, distinguished IPv4 gateway inputs from IPv4/IPv6 usage output, constrained usage provenance, and qualified credential zeroization. The original login stdin buffer now zeroizes on command return and the parsed password zeroizes on every drop path. Final static contract re-review reported no remaining finding.

The aggregate gateway acceptance remains partial/live-unverified. No real usage read, login or logout was authorized or performed; actual endpoint readiness, account permissions, `ip`/`ac_id` values and postconditions remain missing rather than inferred. The parent-required logout plan/commit/idempotency contract is also not implemented by the current one-step typed logout slice. Static source assessment records excluded unsafe upstream defaults and behaviors.
